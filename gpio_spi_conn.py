...
class GPIOSPIConn(object):
    ...
    def setup(self):
        print("Setting up GPIO SPI connection")
        ...
        for attempt in range(0, 3):
            ...
            if res == 0:
                ...
                print("Unable to clear buffer - got unexpected value: {:}".format(res))
    ...
    def recv(self, ack=None):
        ...
        if delay > 0:
            print("Added delay of {:} second between last send and current receive".format(delay))

        # TODO: We always need to receive an ack first to see if we got what we expected?
        if ack != None and ack != ret:
            msg = "Expected ack '{:f}' but got '{:f}'".format(ack, ret)
            print(msg)
